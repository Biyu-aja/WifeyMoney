...
export default function CharacterCard({ character, isSelected, onSelect, onDelete }: Props) {
  ...
  return (
    ...
    >
      {/* Selected indicator */}
      {isSelected && (
        <div className="absolute top-2 right-2 w-6 h-6 gradient-primary rounded-full flex items-center justify-center animate-fade-in">
          <Check size={14} className="text-white" strokeWidth={3} />
        </div>
      )}

      {/* Delete button for custom characters */}
      {!character.isDefault && onDelete && (
        <button
          onClick={(e) => { e.stopPropagation(); onDelete(character.id); }}
          className="absolute top-2 right-2 w-6 h-6 bg-danger/20 rounded-full flex items-center justify-center hover:bg-danger/40 transition opacity-0 group-hover:opacity-100"
